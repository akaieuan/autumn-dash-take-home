 ...
 * panel, chosen by a cookie the server read. Under `sm` it is not on the page at all; the top bar's
 * `MenuButton` opens it as a drawer over the page with a backdrop, because a phone has no room for a
 * column (owner, 2026-09-17). All of that is CSS on two data attributes; nothing here measures the window.
 */
    ...
    <>
      ...
      <aside
        ...
        className={cn(
          ...
          // under sm: gone until the drawer opens, then a fixed panel over the page
          "max-sm:data-[drawer=false]:hidden max-sm:data-[drawer=true]:fixed max-sm:data-[drawer=true]:inset-y-0 max-sm:data-[drawer=true]:left-0 max-sm:data-[drawer=true]:z-40 max-sm:data-[drawer=true]:w-72 max-sm:data-[drawer=true]:border-r max-sm:data-[drawer=true]:border-border max-sm:data-[drawer=true]:shadow-2xl",
        )}
      >
        <div className="flex h-11 items-center gap-2.5 px-1">
          ...
          >
            {/* Under sm the panel is only ever seen as an open drawer, so its button is the X that closes it; the menu button that opens it lives in the top bar. From sm it is the panel-collapse icon. CSS picks; nothing measures the window. */}
