...
export function TrendChart({ metric, granularity, points, prevLabel, lastYearLabel }: TrendProps) {
  ...
  // Line shows a dot per bucket while they can be told apart; Area is a gradient mass with none. That is what separates the two.
  const dots = data.length <= 31 ? { r: 3, strokeWidth: 0, fill: "var(--color-current)" } : false;
  const fillId = `trend-fill-${metric}`;
  ...
  return (
    <figure aria-label={title} className="@container flex min-h-0 flex-1 flex-col gap-3">
          ...
          <ComposedChart data={data} margin={{ top: 8, right: 12, left: 0, bottom: 0 }} barCategoryGap="24%">
            <defs>
              <linearGradient id={fillId} x1="0" y1="0" x2="0" y2="1">
                ...
                <stop offset="100%" stopColor="var(--color-current)" stopOpacity={0.03} />
              </linearGradient>
            </defs>
            ...
            {style === "bars" ? (
              ...
              <Line type={curve} dataKey="current" stroke="var(--color-current)" strokeWidth={2.25} dot={dots} activeDot={{ r: 5, strokeWidth: 2 }} {...anim} />
            ) : (
              <Area type={curve} dataKey="current" stroke="var(--color-current)" fill={`url(#${fillId})`} fillOpacity={1} strokeWidth={2} dot={false} activeDot={{ r: 5, strokeWidth: 2 }} {...anim} />
            )}
      ...
      <figcaption className="flex flex-wrap items-center justify-between gap-2">
        <ChartLegend
          items={[
            { label: trendChartConfig.current.label, kind: style === "line" ? "line" : "rect", color: "var(--chart-1)" },
