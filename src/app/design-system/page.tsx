    ...
    <div className="grid grid-cols-1 gap-(--stack-gap) px-(--page-gutter) py-8 lg:grid-cols-[12rem_minmax(0,1fr)] lg:gap-12">
      ...
      <Stack gap="lg">
        ...
        <DsSection id="organisms" title="Organisms" description="Whole panels as the pages compose them, fed the fixture DTOs. What you see here is what the query layer's shape produces.">
          ...
          <Spec name="ActivityCalendar" file="website-traffic/activity-calendar.tsx" note="Follows the page range (D43), so it has no span control. One column per week, the days at the right and empty cells leading, so a short range fills the row instead of leaving it half empty. Padded to twenty-six columns on a wide panel and thirteen on a narrow one; a longer range draws its own. The band under the grid carries the day, its week and its month."><ActivityCalendar activity={activity} range={activityRange} /></Spec>
