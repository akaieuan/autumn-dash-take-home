...
import { ActivityCalendar, DayCard, MonthBlocks, WeekdayRhythm, TrafficIntro, TrafficMetricSelect, TrafficStory, CampaignTrafficChart, CampaignTrafficTable, EventImpactCard, WhatAutumnDid, CampaignEfficiencyTable, DeviceConversion } from "@/components/website-traffic";
import { Button } from "@/components/ui/button";
import { Badge } from "@/components/ui/badge";
import { Skeleton } from "@/components/ui/skeleton";
import { Tabs, TabsList, TabsTrigger, TabsContent } from "@/components/ui/tabs";
import { ToggleGroup, ToggleGroupItem } from "@/components/ui/toggle-group";
import { range, overview, quick, trend, markets, campaigns, funnel, insights, activity, activityAll, activityRange, campaignSeries, impacts, efficiency, deviceRows } from "./fixtures";
import { monthBlocks, monthContext, weekSummary, weekdayAverages } from "@/lib/activity";
...
const yearBlocks = monthBlocks(activityAll.days, 12);
const allBlocks = monthBlocks(activityAll.days, 24);
const blockMax = (bs: { total: number | null }[]) => Math.max(0, ...bs.map((b) => b.total ?? 0));
const busiestDay = activityAll.days.reduce((b, d) => ((d.value ?? -1) > (b.value ?? -1) ? d : b)).date;
      ...
      <Stack gap="lg">
        ...
        <DsSection id="molecules" title="Molecules" description="Panels and controls. Controls write the URL without scrolling; the page keeps its place.">
          <DsGrid>
            ...
            <Spec name="DayCard" file="website-traffic/day-card.tsx" note="The kept-open day as a band under the grid: the date, its rank and four readings, then a typical day of its weekday, its week and its month as figures. Fixed height (--card-day from sm), so hovering never moves the grid above it." surface="card"><DayCard day={activity.days[activity.days.length - 12]} typical={41} mode="pinned" unit="visits" rank={{ day: 12, days: 90, weekday: 2, weekdays: 13 }} week={weekSummary(activity.days, activity.days[activity.days.length - 12].date)} month={monthContext(activity.days, activity.days[activity.days.length - 12].date)} /></Spec>
            ...
            <Spec name="MonthBlocks" file="website-traffic/month-blocks.tsx" note="Drawn wherever a day square cannot be 12px: year-to-date and twelve months below 42rem of panel, and the whole history at every width. Twelve months 4 × 3, twenty-four 12 × 2 (6 × 4 below 42rem). Oldest top-left; only the roomier three-column grid has space for the change on the month before." surface="card"><div className="@container max-w-sm"><MonthBlocks blocks={yearBlocks} columns={4} unit="visits" pinnedDate={busiestDay} max={blockMax(yearBlocks)} /></div><div className="@container"><MonthBlocks blocks={allBlocks} columns={12} unit="visits" pinnedDate={busiestDay} max={blockMax(allBlocks)} /></div></Spec>
        ...
        <DsSection id="organisms" title="Organisms" description="Whole panels as the pages compose them, fed the fixture DTOs. What you see here is what the query layer's shape produces.">
          ...
          <Spec name="ActivityCalendar" file="website-traffic/activity-calendar.tsx" note="Follows the page range (D41), so it has no span control. One column per week, the days at the right and empty cells leading, so a short range fills the row instead of leaving it half empty. Padded to twenty-six columns on a wide panel and thirteen on a narrow one; a longer range draws its own. The band under the grid carries the day, its week and its month."><ActivityCalendar activity={activity} range={activityRange} /></Spec>
